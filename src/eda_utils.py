...
def plot_continuous_distributions(df: pd.DataFrame, cols: list):
...
def plot_categorical_distributions(df: pd.DataFrame, cols: list, num_columns_grid: int = 3):
...
def plot_bivariate_relationship(df: pd.DataFrame, feature_col: str, target_col: str):
    ...
    axes[1].set_title(f"Volume Distribution Matrix")

    plt.tight_layout()
    plt.show()


def plot_categorical_continuous_matrix(df: pd.DataFrame, categorical_cols: list, continuous_cols: list,
                                      kind: str = "violin", height: float = 4.5, aspect: float = 1.2):
    """
...
def plot_correlation_matrix(df: pd.DataFrame, cols: list, method: str = "pearson"):
    """Computes and renders a lower-triangle correlation heatmap for numeric parameters."""
    corr = df[cols].corr(method=method)
    mask = np.triu(np.ones_like(corr, dtype=bool))

    plt.figure(figsize=(8, 6))
    sns.heatmap(corr, mask=mask, annot=True, cmap="coolwarm", fmt=".2f", center=0, square=True, linewidths=0.5)
